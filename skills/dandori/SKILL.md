...
gate 通過後は annotate（design.md 等の処分で消える Why / Why not をコードコメントとして
保全）→ strip（テスト名の B-ID 等プロセス言及の除去）を経て feedback の安定点に入る —
セッション外のレビューは、dandori の痕跡がないコードに対して受けられる。strip は可逆
（trace.md と git 履歴で B 行↔テスト対応を再現できる）だが、cleanup（フィーチャー
ドキュメント処分・墓碑コミット）は不可逆な店じまいであり、**改訂がもう来ないと確定して
から**行う。その裁定点が dandori-feedback: セッション外のレビュー・運用で決まった
**結論**をユーザーが持ち込んだら改訂サイクル（revision）として巻き戻して再走し、
「実装は完全に fix」の裁定が出たら cleanup → done で閉じる。done 後に結論が来た場合も
dandori-feedback で再開できる（ドキュメント処分済みのため spec を墓碑から復元し、
再 gate は差分トレースになる）。
...
`.dandori/records.md` はクローズ運用の宣言ファイル（タスク置き場・マージ方式・追加キー。
初回クローズまでに annotate 工程が整備する）。フィーチャードキュメントは実装が終わったら
すべて消えるのが既定 — 正はソースコード + テスト、過去の判断は墓碑コミットと ADR。
詳細: `docs/appendix-records.md`
...
取込 / 待機継続 / 完全 fix（cleanup へ）の裁定を仰ぐ。
...
strip:
  status: pending                 # pending | done | skipped（B-ID をコード上に残す裁定 — skip でも feedback へ進む）
cleanup:
  status: pending                 # pending | done（クローズは省略不可 — B-ID 残置の裁定は strip 側）
feedback:
  items: 5                        # 台帳の F 行数（継続改善時のみ）
  trace_scope: delta              # delta | full — 再 gate での旧 B 行の扱い（改訂サイクルで feedback が裁定。strip を skip した B-ID 現役プロジェクトでは書かない）
...
- **長時間ゲートの待ち方**: 全量スイート等の長いゲートをメインエージェントが自分で回すときは、
  ハーネスの背景実行 + 完了通知を使ってよい（ターンを占有し続ける必要はない）。ただし
  **判定は必ず完走した exit code で行う** — 途中出力を見ただけの打ち切り判断は、偽の赤で
  存在しない原因を探させ、偽の緑で未検証を通過させる。サブエージェントにゲートを実行させる
  場合の規約は各工程の workflow が注入する（そちらは背景実行を許さない — 完走の確認が
  報告の自己申告に依存するため）
