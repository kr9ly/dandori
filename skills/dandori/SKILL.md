...
継続改善:     dandori-feedback が裁定点 — 結論の取込（spec | ground | impl に巻き戻し → 再走 → gate → feedback）/ 結論待ちの待機 / 完全 fix の裁定 → dandori-cleanup → done
```
...
（revision）として巻き戻して再走し、「実装は完全に fix」の裁定が出たら cleanup → done で
...
phase: review                     # spec | sketch | ground | review | spike | plan | impl | codereview | refine | gate | feedback | cleanup | done（feedback は gate 後の安定点 — 継続改善の裁定点。cleanup へは feedback の完全 fix 裁定で進む）
