...
**墓碑コミットの直前に機械検査**: 処分対象ファイルに未コミット差分がないことを確認する —
`git status --porcelain -- <処分対象ファイル...>` の出力が空であること。差分があれば
**先に最終版をコミットしてから**削除コミットに進む（2 段を 1 段に潰さない）。
未コミットの最終版ごと削除コミットに巻き込むと、墓碑の親コミットが持つのは古い版になり、
文書の最終版が git 履歴から復元できなくなる（第 5 波 cart-userinfo 実発 —
セッション履歴からの再構築で復旧した実損）。
