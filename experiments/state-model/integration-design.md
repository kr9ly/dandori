...
status: draft（実験3 の成果物 — ユーザー壁打ち前のたたき台）
...
- Covers 構文の異常系対応: 異常系 B 行は E 番号参照が自然（`Covers: E22`）。
  エラーマトリクス側のモデル化（フェーズ×状態、error_order）は実験1 で
  ロールバック横断のみ検証 — E 全体の形式化は過剰かもしれず、次の実運用で判断
- チェッカーの言語: 実験は TS + node。プロジェクト側に node が無い環境の扱い
  （dandori は言語非依存のスキル群 — チェッカーだけ node 依存になってよいか）
