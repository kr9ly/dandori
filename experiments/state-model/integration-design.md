...
- **`ground:` セクションの追加**（2026-07-06 製品化時 — 7 種固定の唯一の例外）:
  裁定済み論点 3「ground 送りを宣言すれば通る」の機械表現。宣言手段がないと
  検査 3 の指摘を人間が黙って容認する運用になり「黙認不可」の思想と矛盾するため、
  `ground: - { axes: [a, b], note: 理由 }`（自己ペア = per-item 混在の裁定送り）を追加。
  チェッカーはペアを裁定済み扱いにしつつ「ground 送り項目」として別枠レポートする
...
指摘とは別枠で「**ground 送り項目**」を出力する（exit code に影響しない — fix は通過可、
gate では残存を要裁定として扱う）: `ground:` 宣言されたペア裁定・per-item 混在裁定
（自己ペア）・base の推定マーク `?`。
...
- **base 推定マーク**（摩擦 2）: `base: S11?` — ground 送り項目として出力
